...
// 2. Bowtie2 固有の上限値定義
def BOWTIE2_MAKEREFDB_MAX_MEMORY  = 32 // GB (FM-index 構築時のメモリ消費を考慮)
def BOWTIE2_MAKEREFDB_MAX_THREADS = 8  // bowtie2-build の並列化飽和点を考慮

def BOWTIE2_ALIGN_MAX_MEMORY      = 32 // GB (インデックス保持＋I/Oバッファ用)
